...
from api.google_api import add_event_to_calendar, check_if_event_collision
...
for task in tasks:
    if check_if_event_collision(task):
        print(f"Event collision detected for task {task.name}")
    else:
